...
for (let item of arr) {
  ...
  for (let j in item) {
    let newArr = [];
    textas = textas + item[j];
    newArr.push(textas);

    for (let i = 0; i < allValidNumber.length; i++) {
      console.log("bbbbbbbbbb", newArr)
      console.log("allValidNum:", allValidNumber[i])
      if (newArr.includes(allValidNumber[i])) {
        newArr = newArr.replaceAll(allValidNumber[i], i + 1); //fgtfhfghone4hjhjhj2
        console.log("newArr newArrzzzzzzzz", newArr);
      }
    }
  }
  debugger;
  ...
  console.log("arrNumber po ciklo:", arrNumber);
